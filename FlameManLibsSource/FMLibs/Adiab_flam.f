       subroutine AdiabFlameTemp (z, komp, c0, temp0, p, symbol, 
     .                            xmol_i, Enth0,ponalo, ponahi,
     .                            nndd,
     .                            adiaT,adiaY)
C z is mixture fraction (not used)
C komp is number of species
C c0 are the initial mass fractions
C temp0 is a guess of the temperature
C p is the pressure in bar
C symbol are the species names, which contains only the composition
C       size of symbol is nOfSpecies+4, strings have the length 20
C xmol_i are the molecular weights in kg/kmole
C Enth0 is the mixture enthalpy in kJ/kg
C ponalo and ponahi are the NASA polynomial coefficients
C nndd is the number of coeffs = 7
C adiaT is the solution for the temperature
C adiaY is the solution for the species mass fractions
       implicit real*8 (a-h, o-z)
       include 'paramtr.h'
       parameter (nwx = (nsx*(3*nsx+13))/2)
       external rhs
       real*8 c0(komp), adiaY(komp), xmol_i(komp),
     .        ponalo(nndd,komp), ponahi(nndd,komp), Enth0,
     .        adiaT, p , temp0 ,z
       real*8 resid(nsx), war(nwx), cn(nsx+nex+1)
       character*20 symbol(komp)
       character foya*20, tab*1, fosy*14,ate*1
       integer checkele
       logical isotherm, intermed, isobar
       common /badiab/     Tad, H0, e0(nex), A(nsx,nsx), nkomp,nele
     .        /bscale/     scale(nsx)
     .        /bmolweight/ xmol(nko)
     .        /brand/      isotherm
     .        /bp0/        ppp
     ...
     .        /transp/     ylast(nsx)
      ...
      common/bspedat/hpch(7,nko),hpcl(7,nko)
      common /kabale/ T2new, T2ad

      if (komp .gt. nko) stop 'KOMP too big'
      if (nndd .ne. 7) stop ' NNDD not fit'
      nele = checkele(symbol,komp)
      ppp = p
C      write (*,*) 'komp = ', komp, ' Z = ', z
       do i =1, komp
       xmol(i) = xmol_i(i)
       do j =1, nndd
       hpch(j,i) = ponahi(j,i)
...
C        write (*,'(a20,7e12.3)') symbol(i),(ponahi(j,i),j=1,nndd)
C        write (*,'(a20,7e12.3)') symbol(i),(ponalo(j,i),j=1,nndd)       
       end do
       tab = char(9)
       lwa = nwx
       nkomp = komp
       neq = komp+nele + 1 + nside
       if (isotherm) neq = komp+nele+nside
       Tad = temp0
       tol = 1.d-12
C       write (*,'('' Kompo, Neq: '', 2i5)') komp, neq
       tr0 = temp0
c-----------------------------
...
c-----------------------------
      ...
      do 5 i = 1, komp
      cn(i) = c0(i)/xmol(i)
C      write (*,'('' Y-i, M_i:'',a8,2g15.6)') symbol(i),c0(i),xmol(i)
       H0 = H0 + cn(i)*Enthalpy (i, temp0, cp, dcpdt)
 5    continue
C      write (*,'('' H0, Enth0 :'',3e15.6)') H0, Enth0, temp0
      H0 = Enth0
       cn(neq) = log(temp0)
c      Tad = temp0
       if (.not. isotherm) then
c      write (*,'('' Input T_ad est '')')
c      read (*,*) Tad
c      Tad = temp0+ 1000.0
       Tad = temp0
       else
c      Tad = temp0
       ...
       call init (komp, cn, temp0, p, symbol)


       call hybrd1 (rhs, neq, cn, resid, tol, info, war, lwa)
       ...
       do i =1, komp
       sum = sum + xmol(i)*exp(cn(i))
       smq = smq + exp(cn(i))
       adiaY(i) = xmol(i)*exp(cn(i))
       end do
       ttt = Tad
       if (.not. isotherm) ttt = exp(cn(neq))

       rho = ppp/(83.134d0*ttt* smq)

c      write (9, 10) (symbol(i), rho*xmol(i)*exp(cn(i)),i=1,komp)
       write (9, 10) (symbol(i),     xmol(i)*exp(cn(i)),i=1,komp)
c      write (9,'('' lambda_i : '',4(1pg10.3))')
c    .               (exp(cn(j+komp)),j=1,nex)
c      write (9, 11) (symbol(i), resid(i), i=1,neq)
 10    format (' Loesung erreicht, Massenbruch:',/,
     .        (3(1x, a20,':',1pe12.4)) )
 11    format ( (3(1x, a20,':',1pe12.4)) )
       write (9,'(''Sum Y_i '',1pe15.7)') sum

       ttt = temp0
       adiaT = exp(cn(neq))
       if(.not. isotherm) ttt = exp(cn(neq))
       write (9, '('' T_0,Phi, p, T_adiabat: '',5f15.5)') temp0, 
     .                z, ppp, ttt
       else

       write (*, 30) (symbol(i), xmol(i)*exp(cn(i)),i=1,komp)
       write (*,'('' Temp     : '',4(1pg10.3))') exp(cn(neq))
       write (*, 31) (symbol(i), resid(i), i=1,neq-1)
       write (*,'('' Error Code : '',i10)') info
 30    format (' Loesung NICHT erreicht, Massenbr.:',/,
     .        (3(1x, a15,':',1pe12.4)) )
 31    format ( (3(1x, a15,':',1pe12.4)) )
       sum = 0.d0
       do i =1, komp
       sum = sum + xmol(i)*exp(cn(i))
       end do
       write (*,'(''Sum Y_i '',1pe15.7)') sum
      ...
      if (info .eq. 1) write (11,'(6e15.6)' ) phi, ttt, t2ad, t2new,
     .                 traw(cn), tfine(cn)
       
 200   continue

C**HP**C       write (55, '(50e12.5)') ttt, (exp(cn(i)),i=1,komp)
       return
       end

       integer function checkele (symbol, komp)

       character*(*) symbol(komp)
       character tech(4)*1
       logical found
       ...
       do j =1, komp
       found = found .or. (index(symbol(j),tech(it)) .gt. 0)
       end do
       if (found) nel = nel + 1
      ...
      double precision function enthalpy (i, t, cp, dcpdt)
      implicit real*8 (a-h,o-z)
      include 'paramtr.h'
      common/bspedat/hpch(7,nko),hpcl(7,nko)
      data Rm/8.3143d0/
...
c---- low temperature case
      ...
      cp = hpcl(5,i)
      do 1 k=1,4
      h=h*t+hpcl(5-k,i)/dfloat(5-k)
      cp = cp*t + hpcl(5-k,i)
    1 continue
      h=h*t+hpcl(6,i)
      dcpdt = hpcl(2,i) + t*(hpcl(3,i)*2.d0 + t*(hpcl(4,i)*3.d0 +
     .         t*4.d0*hpcl(5,i)))
...
c---- high temperature case
    3 continue
      ...
      cp = hpch(5,i)
      do 4 k=1,4
      h=h*t+hpch(5-k,i)/dfloat(5-k)
      cp = cp*t + hpch(5-k,i)
    4 continue
      h=h*t+hpch(6,i)
      dcpdt = hpch(2,i) + t*(hpch(3,i)*2.d0 + t*(hpch(4,i)*3.d0 +
     .         t*4.d0*hpch(5,i)))
    ...
    5 h=h*Rm
      ...
      double precision function entropy (i ,t)
      implicit real*8(a-h,o-z)
      include 'paramtr.h'
      common/bspedat/ hpch(7,nko),hpcl(7,nko)
      data Rm/8.3143d0/
      if (t .ge. 1.d+03) go to 3
c**** low temperature case
      s=hpcl(5,i)*2.5d-01
      do 1 k=1,3
      s=s*t+hpcl(5-k,i)/dfloat(4-k)
    1 continue
      s=s*t+dlog(t)*hpcl(1,i)+hpcl(7,i)
      go to 5
    3 continue
c**** high temperature case
      s=hpch(5,i)*2.5d-01
      do 4 k=1,3
      s=s*t+hpch(5-k,i)/dfloat(4-k)
    4 continue
      s=s*t+dlog(t)*hpch(1,i)+hpch(7,i)
    5 s=s*Rm
      entropy = s
      return
      end
       ...
       subroutine rhs (n, y, resid, iflag)
       implicit real*8 (a-h, o-z)
       include 'paramtr.h'
       real*8 y(n), resid(n)
       real*8 hi(nsx), gi(nsx),cpi(nsx)
       logical isotherm, isobar
       common /badiab/      T0, h0, e(nex), a(nsx,nsx), komp, nele
       common /bscale/      scale(nsx)
     .        /brand/       isotherm
     .        /bp0/         pres
     .        /bmolweight/  xmol(nko)
     .        /bound/       isobar 
     .        /sidecon/     y00(nside+1)
c-------------------------------
c     compute temperature due to constant enthalpy condition
c     the right hand sides : g(i), lambda(j)
c--------------------------------
       if (.not. isotherm) then
       nT = komp+nele+1
       temp = exp(y(nT))
...
c-------------------------------
       if (isobar) then
       call gibbs (komp, temp, y, cpi, hi, gi)
       else
       call helmholtz (komp, temp, y, hi, gi)
       end if

       do 1 i =1, komp
       resid(i) = gi(i) 
       do 1 k = 1, nele
       resid(i) = resid(i) - y(komp+k)*a(i,k)
 1     continue
       
       do 2 j =1, nele
       resid(komp+j) = - e(j)
       do 2 i =1, komp
       resid(komp+j) = resid(komp+j) + a(i,j)*exp(min(600.0D0,y(i)))
 2     continue
       ...
       do 3 i =1, komp
 3     resid(nT) = resid(nT) + hi(i)*exp(min(600.0D0,y(i)))
       resid(nT) = resid(nT)/(8.3143d0*temp)

       if (nside .eq. 1) then
       resid(n) = exp(y(6)) - y00(1)
       end if
       ...
       subroutine gibbs (komp, temp, y, cp, h, g)
       implicit real*8 (a-h, o-z)
       real*8 y(komp), h(komp), g(komp),cp(komp)
       common /bp0/ p

       sum = 0.d0
       do 1 i =1, komp
       h(i) = enthalpy (i, temp, cp(i), dcp)
       g(i) = (h(i)/temp - entropy(i, temp))/8.3143d0 + y(i) +
     .        log(p/1.01325d0)
       sum = sum + exp(min(600.0D0,y(i)))
 1     continue
       do 2 i =1, komp
       g(i) = g(i) - log(sum)
 2     continue
       ...
       subroutine helmholtz (komp, temp, y, u, f)
       implicit real*8 (a-h, o-z)
       real*8 y(komp), u(komp), f(komp)
       common /brho0/ rho0
     .        /bp0/   p

       sum = 0.d0
       do 1 i =1, komp
       u(i) = enthalpy (i, temp, cp, dcp) - 8.3143d0*temp
       f(i) = (u(i) - temp*entropy(i, temp))/8.3143d0/temp + y(i) 
       sum = sum + exp(min(600.0D0,y(i)))
 1     continue
       p   = rho0*83.143d0*temp*sum
       do 2 i =1, komp
       f(i) = f(i) - log(sum) + log(p/1.01325d0)
 2     continue

       return
       end
       
       subroutine McBride (n, y, ynew, icounter, dlambda)
       implicit real*8 (a-h, o-z)
       include 'paramtr.h'
       real*8 y(*), ynew(*)
       real*8 hi(nsx), gi(nsx), cpi(nsx)
       real*8 ama(2+nex,2+nex), b(2+nex), ip(2+nex)
       real*8 ycor(nsx)
       logical isotherm, isobar
       logical loop
       common /badiab/      T0, h0, e(nex), a(nsx,nsx), komp,nele
...
c-------------------------------
       ...
       do 3 i =1, komp
       b(j) = b(j) - a(i,j)*exp(min(600.0D0,y(i))) + 
     .        a(i,j)*exp(min(600.0D0,y(i)))*gi(i)      ! RHS equat 24
 3     continue
 2     continue

       b(nele+1) =  zeron
       do 10 i = 1, komp                   ! RHS equat 26
       b(nele+1) = b(nele+1) + exp(min(600.0D0,y(i)))*(gi(i) -1.0d0)
 10    continue
       b(nele+2) = h0
       do 20 i =1, komp                    ! RHS equat 27
           b(nele+2) = b(nele+2) - hi(i)*exp(min(600.0D0,y(i))) +
     .             exp(min(600.0D0,y(i)))*gi(i)*hi(i)
 20    continue
          b(nele+2) = b(nele+2)/(Rm*Temp)
       ...
       sum = 0.0
       do 102 j = 1, komp
          sum =sum+a(j,k)*a(j,i)*exp(min(600.0D0,y(j)))  ! AMA equat 24
 102   continue
       ama(k,i) = sum
 101   continue
       ...
       sum2= 0.0
       do 110 j =1, komp
          sum = sum + a(j,k)*exp(min(600.0D0,y(j)))*hi(j)
          sum2 = sum2 + a(j,k)*exp(min(600.0D0,y(j)))
 110   continue
       ama(k,nele+2) = sum/(Rm*Temp)
       ama(k,nele+1) = sum2
 ...
 100   continue
       ...
       do 120 i = 1, nele
        sum = 0.0
         do 121 j =1, komp
            sum = sum + a(j,i)*exp(min(600.0D0,y(j)))
 121   continue
       ama(nele+1,i) = sum               ! AMA 26
 120   continue

       sum = 0.0d0
       do 125 j = 1, komp
       sum = sum + exp(min(600.0D0,y(j)))
 125   continue
       
       ama(nele+1,nele+1) = sum - zeron


       sum = 0.0
       do 130 j = 1, komp
        sum = sum + exp(min(600.0D0,y(j)))*hi(j)
 130   continue

       ama(nele+1,nele+2) = sum/(Rm*Temp)

       do 150 i =1, nele

       sum = 0.0
       do 151 j =1, komp
          sum = sum + a(j,i)*exp(min(600.0D0,y(j)))*hi(j)
 151   continue
       ama(nele+2,i) = sum/(Rm*Temp)
 150   continue
       ...
       do 155 j = 1, komp
          sum = sum + exp(min(600.0D0,y(j)))*hi(j)
 155   continue
       ama(nele+2,nele+1) = sum/(Rm*Temp)

       sum = 0.0
       do 160 j = 1, komp
        sum=sum+exp(min(600.0D0,y(j)))*(cpi(j)/Rm+(hi(j)/(Rm*Temp))**2)
 160   continue
  
       ama(nele+2,nele+2) = sum
       
       nadim = nex+2
       nprob = nele + 2
c             do i =1, nprob
c            write (*,'(6e13.5)') b(i)
c             write (*,'(6e13.5)') (ama(i,j),j=1,nprob)
c            end do
          
       call dgefa ( ama, nadim, nprob,ip, info)

           
       call dgesl(ama, nadim, nprob,ip, b, 0)
          
CD        write (*,*) 'INFO : ',info
CD         write (*,'(5e12.5)') (b(i),i=1,nele)
CD        write (*,'(5g15.5)') (b(i),i=nele+1,nele+2)
          if (info .ne. 0) then
          write (*,*) info
          stop ' Error in DGEFA ' 
          end if

          Tnew = exp( min(600.0D0,log(Temp) + b(nele+2)))
CD         write (*,'('' T: '',f15.5,5x,'' TNew:'',g15.7)') Temp,Tnew

       dlam1 = max(abs(b(nele+2)), abs(b(nele+1)))
       ente = 0.0d0
       do j =1, komp
           ente = ente + exp(min(600.0D0,y(j)))
          end do

c
c    Species corrections
c
       ente = zeron+1.0D-60
       do j =1, komp
       sum = 0.0
       ...
       ycor(j) = -gi(j) + b(nele+2)*hi(j)/(Rm*Temp)
     .           + b(nele+1) + sum
c      yw = exp( min(600.0D0,y(j) + ycor(j)))
c      write (*,33) j, exp(y(j))/max(1.0d-10,yw)
c      if (ycor(j) .gt. 0.0d0 .and. exp(y(j))/ente .gt. 1.0d-8)
       if (exp(min(600.0D0,y(j)))/ente .gt. 1.0d-8)
     .          dlam1 = max(dlam1, abs(ycor(j)))
       end do
       dlam1 = 1.0d0/dlam1
c       write (*,*) dlam1

       dlam2 = 1.0d0
          teslo = 1.00000000d-08
          telog = log(teslo)
       do 200 j =1, komp
c      if(exp(y(j))/ente .le. teslo .and. ycor(j) .gt. 0.0d0)
       if(exp(min(600.0D0,y(j)))/ente .le. teslo )
     .   dlam2 = min(dlam2, abs((-log(exp(min(600.0D0,y(j)))
     .       /ente+1.0D-60) + telog/2.0d0)/( ycor(j) - b(nele+1))))
 200   continue
      
       dlmin = 1.0d0
       if (icounter .lt. 20) dlmin=0.01d0
       dlambda2 =  min(dlmin, dlam1 , dlam2)
       if (dlambda2 .gt. 1.6d0*dlambda) then
        dlambda = 1.6d0*dlambda
        else
        dlambda = dlambda2
        end if

c       write (*,*)' lambda total ', dlambda, icounter,dlam1, dlam2

       do 230 j = 1, komp
        ynew(j) = y(j) + dlambda*ycor(j)
c       write (*,'(i20,5e15.8)')  j,
c    .             xmol(j)*exp(y(j)),xmol(j)*exp(ynew(j))
 230    continue

       ynew(nT) = y(nT) + dlambda*b(nele+2)
       zeron = zeron *exp( min(600.0D0,dlambda*b(nele+1)) )
c      write (*,'(a5,5e15.8)')'Temp ', exp(y(nT)),exp(ynew(nT))
       if (loop) then
       icounter = icounter + 1
       sum = 0.0d0
       do 300 j =1, komp
 ...
 300    continue
       ...
       do 301 j =1, komp
       y(j) = ynew(j)
       delmax = max(delmax, exp(min(600.0D0,ynew(j)))*abs(ycor(j))/sum)
 301    continue
       do 303 k =1, nele
 303      y(k+komp) = 1.0d0
       y(nT) = ynew(nT)
       if (delmax .gt. 0.50d-05 .and. icounter .lt. 1000) goto 1000
c       write (*,*) ' I_Counts ', icounter , delmax
        end if
 ...
 33    format (i5, 5e15.5)
      ...
      subroutine init (komp, cn, temp0, p, symbol)
      implicit real*8 (a-h, o-z)
      include 'paramtr.h'
      real*8 cn(komp)
      real*8 ym(1000), cpp(nsx), war(1000), y2(1000),
     .       resi(1000), cnew(1000)
      character*(*) symbol(komp)
      character*1 element(4)
      external tomcat, secdim
      real*8 lasty
      logical isotherm, isobar

      common /badiab/ Tad, H0, E(nex), a(nsx,nsx), nkomp,nele
     .       /brand/ isotherm
     .       /bound/  isobar
     .       /bmolweight/ xmol(nko)
     .       /coh/        ioh
     .       /nullpo/       zeron
       common /five/ y5(200), hk0, ip2(200), kkomp
       common /kabale/ T2new, T2ad
     .        /sidecon/    y00(nside+1)
      common /transp/ lasty(nsx)
      

      data element /'H', 'O', 'N', 'C' /


      do 1 j =1, nele
      do 2 i =1, komp
      lesy = index(symbol(i),' ') - 1
      a(i,j) = 0.d0
      iasum  = 0
      l0 = index(symbol(i)(1:lesy),'-') + 1

  55  continue
  
      ni = index(symbol(i)(l0:lesy) , element(j) ) + l0 -1
c===========================================================
      if (ni .gt. l0 - 1) then
       itest = ichar(symbol(i)(ni+1:ni+1)) 
       if (itest .ge. 48 .and. itest .lt. 58) then
        ipp = 1
        itest2 = ichar(symbol(i)(ni+2:ni+2)) 
        if (itest2 .ge. 48 .and. itest2 .lt. 58) then
         ipp = 2
         read (symbol(i)(ni+1:ni+2),'(i2)') ia
        else
         read (symbol(i)(ni+1:ni+1),'(i1)') ia
        end if
       else
       ia = 1
       end if
...
c============================================================      
      if (iasum .gt. 0 )then
      if (iasum .gt. 1) then     
C       write (*,'(a20,'' hat '',i2,1x,a1,'' Atome'')')symbol(i), iasum,
C     .      element(j)
      else
       ia = 1
C       write (*,'(a20,'' hat '',i2,1x,a1,'' Atom  '')')symbol(i), iasum,
C     .      element(j)
      endif
      end if
 ...
 1    continue
      ...
      do 3  j = 1, nele
      E(j) = 0.d0
      symbol(komp+j)(1:1) = element(j)
      do jj = 2, 20
       symbol(komp+j)(jj:jj) = ' '
       end do
      do 3 i = 1, komp
      ym(i) = cn(i)
 3    E(j) = E(j) + cn(i)*a(i,j)

      do 4 i =1, komp
      cn(i) = log( 0.01d0/dfloat(komp) + ym(i) ) 
 4    continue
...
C     print *,' H_0 : ', H0
      ...
      do 7 j =1, nele
 7    cn(komp+j) = dfloat(j)

      sum = 0.0
      zeron = 0.10d0
      do 10 j = 1, komp
 10    sum = sum + exp(cn(j))*xmol(j)

c     do 11 j = 1, komp
c11     cn(j) = log (  exp(cn(j)) /sum )
      ...
      if (imb .eq. 0) then
         call McBride (neq, cn, cnew, imb, dlambda)
      else
         
         do 200 i = 1, imb
         call McBride (neq, cn, cnew, i, dlambda)
         sum = 0.000d0
         do 199 j = 1, komp
          sum = sum + exp(cnew(j))*xmol(j)
 199   continue
c     write (*,*) i,' SUmme is ',sum
         Tomc = exp(  cn(neq) )
          do 201 j = 1, komp
           cn(j) = cnew(j) ! - log(sum)
 201  continue
      if (i .gt. 0)  cn(neq) = cnew(neq)
         do 202 j = 1, nele
          cn(komp+j) = 1.0d0
 202  continue
...
c     sum = 0.d0
c     do j = 1, komp
c         write (*,*) symbol(j), exp(cn(j))*xmol(j)
c         sum = sum + xmol(j)*exp(cn(j))
c         end do
c     sum = 1.0d0
...
c      end do
      ...
      return
       ...
       subroutine secdim (n, y, resid, iflag)
       implicit real*8 (a-h, o-z)
       include 'paramtr.h'
       real*8 y(n), resid(n)
       real*8 hi(nsx), gi(nsx),cpi(nsx)
     ...
     .        /bound/       isobar 
     .        /sidecon/     y00(nside+1)
      common /five/ y5(200), hh0, ip2(200), kkomp
c-------------------------------
c     compute temperature due to constant enthalpy condition
c     the right hand sides : g(i), lambda(j)
c--------------------------------
...
c-------------------------------
       ...
       if (isobar) then
       call gibbs (komp, temp, y5, cpi, hi, gi)
       else
       call helmholtz (komp, temp, y5, hi, gi)
       end if

       do 1 i =1, n-nele-1
       resid(i) = gi(ip2(i)) 
       do 1 k = 1, nele
       resid(i) = resid(i) - y(komp+k)*a(ip2(i),k)
 1     continue
       
       do 2 j =1, nele
       resid(komp+j) = - e(j)
       do 2 i =1, n-nele-1
       resid(komp+j)=resid(komp+j)+a(ip2(i),j)*exp(min(600.0D0,y(i)))
 2     continue
       ...
       resid(nT) = -h0
       do 3 i =1, n-nele-1
 3     resid(nT) = resid(nT) + hi(ip2(i))*exp(min(600.0D0,y(i)))
       resid(nT) = resid(nT)/(8.3143d0*temp)
      ...
      subroutine tomcat (n, y, resid, ifalg)
      implicit real*8 (a-h, o-z)

      real*8 y(n), resid(n)
      common /five/ y5(200), h0, ip2(200), komp

      cp0 = 0.0
      h1 = 0.0d0

      do 1 i = 1, komp
       h1 = h1 + y5(i)*enthalpy(i, y, cp, dcp)
       cp0 = cp0 + cp*y5(i)
 1    continue
...
c 
      ...
      double precision function dpmpar (k)
      integer k
...
c     **********
     ...
     *                 x1max,x3max,zero 
      data one,zero,rdwarf,rgiant /1.0d0,0.0d0,3.834d-150,1.304d150/
      s1 = zero 
      s2 = zero 
      s3 = zero 
      x1max = zero
...
c 
      ...
      double precision function traw(yl)

      implicit real*8 (a-h, o-z)
      include 'paramtr.h'
      real*8  yl(*)
      
      common /hraw/ hi(nsx,3)
      common /badiab/ Tad, H0, E(nex), a(nsx,nsx), komp,nele

      sum0 = 0.0
      sum1 = 0.0d0

      do 1 i =1, komp
       sum0 = sum0 + exp(yl(i))*hi(i,1)
       sum1 = sum1 + exp(yl(i))*hi(i,2)
 1    continue

      traw = (H0 - sum0)/(sum1+1.0D-60)
      traw = 1.0

      return
      end

      double precision function tfine(yl)
      ...
      do 1 i =1, komp
       sum0 = sum0 + exp(yl(i))*hi(i,1)
       sum1 = sum1 + exp(yl(i))*hi(i,2)
       sum2 = sum2 + exp(yl(i))*hi(i,3)
 1    continue

      tfine = (-sum1 + sqrt(sum1*sum1 - 4.0d0*sum0*sum2))/
     .        (2.0d0*sum0+1.0D-60)
