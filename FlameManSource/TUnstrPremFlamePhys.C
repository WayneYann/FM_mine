...
void TUnstrPremFlamePhys::InitTUnstrPremFlamePhys( void )
{
  ...
  if (fSoot)
  {
    ...
    for (k = 0; k < nMoments; ++k)
    {
      ...
      fVariableNames[offset+k] = new char[14];
      if (!fVariableNames[offset+k]) FatalError("new failed");
      sprintf(fVariableNames[offset+k], "M%d_%d_%d/rho", i, j, l);
    }
  }
#endif
#ifdef VS
  ...
  if (fSoot)
  {
    ...
    for (k = 0; k < nMoments; ++k)
    {
      ...
      fVariableNames[offset+k] = new char[8];
      if (!fVariableNames[offset+k]) FatalError("new failed");
      sprintf(fVariableNames[offset+k], "M%d/rho", i);
    }
  }
#endif
...
void
TUnstrPremFlamePhys::SetTemperature (MatrixPtr yMat, double *yLeft,
				     double *yRight)
{
  ...
  yLeft[fTemperature] = InterpolOne (0, fFixX, fFixTemp, fNFixTemp);
  ...
  yRight[fTemperature] = y[nGridPoints - 1][fTemperature];
}

void TUnstrPremFlamePhys::SetMassFlux (Double massFlux, MatrixPtr yMat, Double *yLeft, Double *yRight)
{
	int k;
	int nGridPoints = yMat->cols;
	Double **y = yMat->mat;
	...
	Double h, hm, rhodot;
	//density correction
	//=>the Mass flux is not constant anymore
	//=>integrate the density correction through the domain
	y[fTfixLoc][fMassFlowRate] = massFlux;

	for (k = fTfixLoc + 1; k < nGridPoints; ++k)
	{
		bt->SetNodeInfo (this, k);
		SetFlameNode (k);
		hm = nodeInfo->hm;
		if (GetSoot ())
		{
			rhodot = fFlameNode->rhodot[kCurr];
			y[k][fMassFlowRate] = y[k - 1][fMassFlowRate] + hm * rhodot;
		}
		else
	...
	{
		rhodot = fFlameNode->rhodot[kCurr];
		yRight[fMassFlowRate] = y[nGridPoints - 1][fMassFlowRate] + hm * rhodot;
	}
	else
		yRight[fMassFlowRate] = massFlux;

	for (k = fTfixLoc - 1; k >= 0; --k)
	{
		bt->SetNodeInfo (this, k);
		SetFlameNode (k);
		h = nodeInfo->h;
		...
		{
			rhodot = fFlameNode->rhodot[kCurr];
			y[k][fMassFlowRate] = y[k + 1][fMassFlowRate] - h * rhodot;
		}
		else
			y[k][fMassFlowRate] = massFlux;
	}
	...
	{
		rhodot = fFlameNode->rhodot[kCurr];
		yLeft[fMassFlowRate] = y[0][fMassFlowRate] - h * rhodot;
	}
	else
...
void UnstrPremPhysJacLast( void *object, NodeInfoPtr nodeInfo )
{
	...
	/*		for ( int i = 0; i < nSootMom; ++i ) {
				...
				Double	fracMom = flame->GetSoot()->FracMom2( fracIndex, moments );
				for ( int j = 0; j < nSootMom; ++j ) {
					a[j+sootoff][rOff] -= fact * flame->GetSoot()->GetAlphaI2( j, fracIndex ) 
											 * mixDensity[kCurr] / moments[j];
				}
			}*/
		}
		else {
...
//			rhs[fMassFlowRate] += 0.0;		// if no energy equation is solved, don't change M
		}
	}
	else if ( nodeInfo->gridPoint < flame->fTfixLoc ) {
		rhs[fMassFlowRate] -= FirstDerivUpwind( yNext[fMassFlowRate], y[fMassFlowRate], h ) + rhodot;
//		rhs[fMassFlowRate] -= FirstDerivUpwind( yNext[fMassFlowRate], y[fMassFlowRate], h );
	}
	else if ( nodeInfo->gridPoint > flame->fTfixLoc ) {
		rhs[fMassFlowRate] += FirstDerivUpwind( y[fMassFlowRate], yPrev[fMassFlowRate], hm ) - rhodot;
...
/*		rhs[speciesEq] -= flame->SecondDerivXDiffusion( speciesEq, nodeInfo );*/
/*#	endif*/
/*#	ifdef DIFFUSIVITYCORRECTION*/
/*		if ( flame->UseDiffCorr() ) {*/
/*			rhs[speciesEq] += flame->DiffCorr( speciesEq, nodeInfo );*/
/*#		ifdef MOLARDIFFUSION*/
/*			rhs[speciesEq] += flame->DiffCorrX( speciesEq, nodeInfo );*/
/*#		endif*/
/*		}*/
/*#	endif*/
/*	}*/
...
#	endif
		...
		if ( flame->fProperties->GetRadiation() ) {
		  rhs[fTemperature] -= flame->fRadFact * oneOverCp * flameNode->radiation[kCurr]; //SD
			if ( flame->GetSoot() && flame->GetSoot()->WithSootRadiation() ) {
//				if ( flame->GetSoot()->GetSootRadiation( y[fTemperature], flameNode->moments ) < 0.0 ) {
//					fprintf( stderr, "sootradiation lower than zero: temp = %g, moment1 is %g\n"
//							, y[fTemperature], flameNode->moments[1] );
//				}
			  rhs[fTemperature] += oneOverCp * flame->GetSoot()->GetSootRadiation( y[fTemperature], flameNode->moments );  //SD
			  //				fprintf( stderr, "rad = %g\tsootrad = %g\ttemp = %g\n"
			  //		, flameNode->radiation[kCurr]
			  //		, flame->GetSoot()->GetSootRadiation( y[fTemperature], flameNode->moments )
			  //		, y[fTemperature] );
			}
		}
	...
	//Temperature - Profile is Specified: Added by MM
...
//	update properties
	...
	if (flame->GetImposeTemp() && !flame->GetRelaxTemp())
	{
	  flame->SetTemperature(currGrid->GetY(), currGrid->GetYLeft()->vec, currGrid->GetYRight()->vec);
	}

	if ( flame->fConstMassFlux ) {
//		Double	mixMolarMass = flame->fProperties->GetMolarMass()->vec[kPrev];
...
//		fprintf( stderr, "mixMolarMass(left) = %g\trho298 = %g\tpressure = %g\tmassflux = %g\n"
//						, mixMolarMass, rho298, GetPressure(), fConstMassFlowRate );
	}
...
//	if ( 0 && iDidit == 0 && ( i = flame->fSpecies->FindSpecies( "CH" ) ) >= 0 ) {
//	        iDidit = 1;
//		flame->fTfixLoc = LocationOfMax( nGridPoints, &Y[kCurr][i], nSpecies );
//	}
//	else {
	...
	//deltaTFix = 500;
	//printf("delta = %g\t orig = %g\n",deltaTFix,flame->fDeltaTFix);
		for ( i = 0; i < nGridPoints; ++i ) {
		  //			if ( temp[i] > ( yLeft[fTemperature] + 1100.0/*flame->fDeltaTFix*/ ) ) {
...
{
  int 			i, j, k, hh, l, m, n, fromone, stop;
	TBVPSolverPtr		solver = GetSolver();
	TNewtonPtr		bt = solver->bt;
	NodeInfoPtr		nodeInfo = bt->GetNodeInfo();
...
// debugging ATTENTION HP
	...
	if ( param ) {
		...
		fprintf( stderr, "%s%g%s\n", "initial debug pressure is ", GetPressure()/1.0e5, " bar"  );
	}
	else { // exit
...
#ifdef DEBUGINIT
	fprintf( stderr, "left = %g\n", left );
	fprintf( stderr, "right = %g\n", right );
	for ( j = 0; j < variables; ++j ) {
		fprintf( stderr, "yLeft[%d] = %g\n", j, yLeft[j] );
		fprintf( stderr, "yRight[%d] = %g\n", j, yRight[j] );
	}
	for ( i = 0; i < nGridPoints; ++i ) {
		fprintf( stderr, "locX[%d] = %g\n", i, locX[i] );
	}
#endif
// set default values
	for ( i = 0; i < nGridPoints; ++i ) {
...
//			fprintf( stderr, "read massflowrate\n" );
		}
...
/*			for (l = 0; l <= 6*GetSoot()->Geti(GetSoot()->GetNSootMoments()-1); l+=6)
			{
				sprintf(name, "%d", l);
				if (strncmp(name, string, 3) == 0)
				{
					variable = GetSoot()->GetInvi(l) + GetSoot()->GetOffsetSootMoments();
					if (l!=(GetSoot()->Geti(GetSoot()->GetNSootMoments()-1)-1))
					fromone = 1;
					stop = l;
					string +=1;
				}
			...
			for (m = 0; m < GetSoot()->GetNSootMoments(); ++m)
			{
				...
				j = GetSoot()->Getj(m);
				hh = GetSoot()->Getk(m);
				sprintf(name, "%d-%d-%d", l, j, hh);
					if (strncmp(name, string, 5) == 0)
					{
		...
		if ( chooseInputGrid ) {
			for ( k = 0; k < gridPointsIn-2; ++k ) {
				...
				if (fromone)
				{
					for (n = 1; n <= stop; ++n)
					{
						y[k][variable+n] = yInFloat[i*gridPointsIn+k+1];
					}
				}
			}
		}
...
//	update properties
	...
	if ( fConstMassFlux && fStrainRate && fStrainRate->vec[0] > 0.0 ) { // start value for massflux speciefiled
		...
		fProperties->ComputeMixtureMolarMass( 
				...
				, fSpecies->GetMolarMass()->vec, nSpeciesInSystem );
//		Double	mixMolarMass = fProperties->GetMolarMass()->vec[kPrev];
	...
	if ( GetSoot() ) {
		if ( sootin == kMassFracs ) {
			...
			for ( k = 0; k < nGridPoints; ++k ) {
				...
				for ( j = 0; j < GetSoot()->GetNSootMoments(); ++j ) {
				  #ifdef SOLVELOG
				    y[k][j + GetSoot()->GetOffsetSootMoments()] = log(y[k][j+GetSoot()->GetOffsetSootMoments()]/rho);
				  #else
				    y[k][j + GetSoot()->GetOffsetSootMoments()] /= rho;
				  #endif
				}
...
//	update properties
	...
	if (GetImposeTemp() && !GetRelaxTemp())
	{
	  SetTemperature(fine->GetY(), fine->GetYLeft()->vec, fine->GetYRight()->vec);
	}
...
// write header
	...
	if ( flame->GetSoot() ) {
		fprintf( fp, "SootDASSL = \"%s\"\n", ( flame->GetSoot()->WithSootDASSL() ) ? "TRUE" : "FALSE" );
		...
		fprintf( fp, "Fragmentation = \"%s\"\n", ( flame->GetSoot()->WithFragmentation() ) ? "TRUE" : "FALSE" );
...
// //	write element mass fractions
// 	fprintf( fp, "Z_O\n" );
// 	for ( k = 0; k < gridPoints+2; ++k ) {
// 	  fprintf( fp, "\t%-.6e", flame->GetElementMassFraction(massFracs[k-1], "O", 16.0) );
// 		if ( (k+1) % 5 == 0 ) {
// 			fprintf( fp, "\n" );
// 		}
// 	}
...
// 	fprintf( fp, "Z_C\n" );
// 	for ( k = 0; k < gridPoints+2; ++k ) {
// 	  fprintf( fp, "\t%-.6e", flame->GetElementMassFraction(massFracs[k-1], "C", 12.01) );
// 		if ( (k+1) % 5 == 0 ) {
// 			fprintf( fp, "\n" );
// 		}
// 	}
...
// 	fprintf( fp, "Z_H\n" );
// 	for ( k = 0; k < gridPoints+2; ++k ) {
// 	  fprintf( fp, "\t%-.6e", flame->GetElementMassFraction(massFracs[k-1], "H", 1.008) );
// 		if ( (k+1) % 5 == 0 ) {
// 			fprintf( fp, "\n" );
// 		}
// 	}
...
// 	fprintf( fp, "Effective Equivalence Ratio\n" );
// 	for ( k = 0; k < gridPoints+2; ++k ) {
// 	  fprintf( fp, "\t%-.6e", (flame->GetElementMassFraction(massFracs[k-1], "C", 12.01)+flame->GetElementMassFraction(massFracs[k-1], "H", 1.008))/flame->GetElementMassFraction(massFracs[k-1], "O", 16.0) / ((12.01+4*1.008)/(4*16.0)) );
// 		if ( (k+1) % 5 == 0 ) {
// 			fprintf( fp, "\n" );
// 		}
// 	}
...
//		if ( flame->fRadFact > 2.0 / 7.0 ) {
//			flame->fRadFact *= 0.98;
//			fprintf( stderr, "radfact is now %g\n", flame->fRadFact );
...
FILE *TUnstrPremFlamePhys::GetOutputFile( char *head, char *tail, FileType type )
{
	...
	if ( fConstMassFlux ) {
#ifdef UQ
		sprintf( name, "%s%s%.8s_p%.2d_%.1dphi%.1d_%.4dtu%.4dv%.4d_%s%s"
						, ( head ) ? head : "", ( head ) ? "_" : ""
...
#ifdef UQ
		sprintf( name, "%s%s%.8s_p%.2d_%.1dphi%.1d_%.4dtu%.4d_%s%s"
						, ( head ) ? head : "", ( head ) ? "_" : ""
						, speciesNames[fuelIndex]
						, ( int ) floor( press )	// in [bar]
						, ( int ) ( ( press - ( floor( press ) ) ) * 10 + 0.5 )
...
void TUnstrPremFlamePhys::SetInitialBC( TGridPtr grid, TInputDataPtr inp )
{
	...
	if (left->fMixtureSpecification == kMolarFraction || left->fMixtureSpecification == kMassFraction)
	{
	  SetMixtureSpecificationLeft (kMassFraction);
	}
...
void UnstrPremPhysUpdateLeftBoundary( void  *object )
{
	...
	flame->ComputeProperties( flame->fFlameNode, y[0][flame->GetOffsetTemperature()]
							, flame->fFlameNode->Y[kCurr], flame->GetPressure() );
	Double			mixDensity0 = flame->fProperties->GetDensity()->vec[0];
	Double			*diffusivity0 = flame->fSpecies->GetDiffusivity()->mat[0];
	Double			coeff;
		for ( int i = 0; i < nSpeciesInSystem; ++i ) {
			coeff = mixDensity0 * diffusivity0[i] / ( yLeft[fMassFlowRate] * hm );
//			coeff = mixDensity * diffusivity[i] / ( yLeft[fMassFlowRate] * hm );
			yLeft[speciesOff+i] = ( bcLeft[speciesOff+i] + coeff * y[0][speciesOff+i] )
							  / ( 1.0 + coeff );
		}
#else
		flame->CalcYLeft();
/*			fprintf( stderr, "YC2H4Dir = %g\tYNewto = %g\n\n", Yi[23], yLeft[speciesOff+23] );*/
/**/
/*fprintf( stderr, "bcLeft[fFirstSpecies+i] = %g\n", bcLeft[speciesOff+23] );*/
/*fprintf( stderr, "rho = %g\n", mixDensity0 );*/
/*fprintf( stderr, "D = %g\n", diffusivity0[23] );*/
/*fprintf( stderr, "Y0 = %g\n", y[0][speciesOff+23] );*/
/*fprintf( stderr, "hFirst = %g\n", hm );*/
/*fprintf( stderr, "M = %g\n", yLeft[fMassFlowRate] );*/
/*fprintf( stderr, "YL = %g\n", Yi[23] );*/
/**/
/*		exit(2);*/
  ...
  if (flame->GetSoot())
  {
...
#endif
    {
      ...
      l = double(flame->GetSoot()->Getk(k)/6.0);
			
      if (k==0)
	yLeft[sootOff] = 1.0e-20;
      else
	yLeft[k+sootOff] = pow(2.0*flame->GetSoot()->nucl_nbrC2, i+j*(2.0/3.0)) * pow(2.0*flame->GetSoot()->nucl_nbrH, l) * yLeft[sootOff];
    }
#ifdef HMOM
    yLeft[sootOff+nSootMoments-1] = 1.0e-20;
#endif
...
void UnstrPremPhysUpdateLeftBoundary(void  *object)
{
  ...
  if (mixtureSpecificationLeft == kMassFlux && y[0][fMassFlowRate] > 0.0)
  {
#ifdef SIMPLEDIFFBC
    flame->SetFlameNode(0);
    flame->ComputeProperties(flame->fFlameNode, y[0][flame->GetOffsetTemperature()], flame->fFlameNode->Y[kCurr], flame->GetPressure());
    Double mixDensity0 = flame->fProperties->GetDensity()->vec[0];
    Double * diffusivity0 = flame->fSpecies->GetDiffusivity()->mat[0];
    Double coeff;
    ...
    for (int i = 0; i < nSpeciesInSystem; ++i)
    {
      coeff = mixDensity0 * diffusivity0[i] / (yLeft[fMassFlowRate] * hm);
//       coeff = mixDensity * diffusivity[i] / (yLeft[fMassFlowRate] * hm);
      yLeft[speciesOff+i] = (bcLeft[speciesOff+i] + coeff * y[0][speciesOff+i]) / (1.0 + coeff);
    }
#else
    flame->CalcYLeft();
#endif
  ...
  if (flame->GetSoot())
  {
...
#endif
    {
      ...
      {
#ifdef SOLVELOG
	yLeft[sootOff] = log(1.0e-20 / mixDensity);
#else
	yLeft[sootOff] = 1.0e-20 / mixDensity;
#endif
      }
      else
      {
...
#ifdef HMOM
  #ifdef SOLVELOG
    yLeft[sootOff+nSootMoments-1] = log(1.0e-20 / mixDensity);
  #else
    yLeft[sootOff+nSootMoments-1] = 1.0e-20 / mixDensity;
  #endif
#endif
#ifdef FRAGTEST
  #ifdef SOLVELOG
//     #ifdef PHI_0_85
//       yLeft[sootOff  ] = log(2.0804e-13 / 1.043797);
//       yLeft[sootOff+1] = log(1.4667e-05 / 1.043797);
//       yLeft[sootOff+2] = log(3.1108e-07 / 1.043797);
//       yLeft[sootOff+3] = log(1.6877e+04 / 1.043797);
//       yLeft[sootOff+4] = log(2.6058e+02 / 1.043797);
//       yLeft[sootOff+5] = log(4.1942e+00 / 1.043797);
//       yLeft[sootOff+6] = log(8.4762e-15 / 1.043797);
//     #endif
//     #ifdef PHI_0_95
//       yLeft[sootOff  ] = log(2.1810e-13 / 1.035113);
//       yLeft[sootOff+1] = log(1.5376e-05 / 1.035113);
//       yLeft[sootOff+2] = log(3.2613e-07 / 1.035113);
//       yLeft[sootOff+3] = log(1.7694e+04 / 1.035113);
//       yLeft[sootOff+4] = log(2.7319e+02 / 1.035113);
//       yLeft[sootOff+5] = log(4.3971e+00 / 1.035113);
//       yLeft[sootOff+6] = log(8.8862e-16 / 1.035113);
//     #endif
//     #ifdef PHI_1_05
//       yLeft[sootOff  ] = log(2.2552e-13 / 1.028150);
//       yLeft[sootOff+1] = log(1.5899e-05 / 1.028150);
//       yLeft[sootOff+2] = log(3.3722e-07 / 1.028150);
//       yLeft[sootOff+3] = log(1.8295e+04 / 1.028150);
//       yLeft[sootOff+4] = log(2.8247e+02 / 1.028150);
//       yLeft[sootOff+5] = log(4.5466e+00 / 1.028150);
//       yLeft[sootOff+6] = log(9.1884e-16 / 1.028150);
//     #endif
//     #ifdef PHI_1_10
//       yLeft[sootOff  ] = log(2.3787e-13 / 1.020561);
//       yLeft[sootOff+1] = log(1.6770e-05 / 1.020561);
//       yLeft[sootOff+2] = log(3.5569e-07 / 1.020561);
//       yLeft[sootOff+3] = log(1.9298e+01 / 1.020561);
//       yLeft[sootOff+4] = log(2.9795e+02 / 1.020561);
//       yLeft[sootOff+5] = log(4.7956e+00 / 1.020561);
//       yLeft[sootOff+6] = log(9.6916e-16 / 1.020561);
//     #endif
//     #ifdef PHI_1_15
//       yLeft[sootOff  ] = log(2.7136e-13 / 1.002392);
//       yLeft[sootOff+1] = log(1.9131e-05 / 1.002392);
//       yLeft[sootOff+2] = log(4.0577e-07 / 1.002392);
//       yLeft[sootOff+3] = log(2.2014e+04 / 1.002392);
//       yLeft[sootOff+4] = log(3.3989e+02 / 1.002392);
//       yLeft[sootOff+5] = log(5.4708e+00 / 1.002392);
//       yLeft[sootOff+6] = log(1.1056e-15 / 1.002392);
//     #endif
    #ifdef PHI_0_85
      yLeft[sootOff  ] = log(1.6354e-12 / 1.050789);
      yLeft[sootOff+1] = log(5.4311e-05 / 1.050789);
      yLeft[sootOff+2] = log(5.8471e-07 / 1.050789);
      yLeft[sootOff+3] = log(3.3149e+03 / 1.050789);
      yLeft[sootOff+4] = log(3.4201e+01 / 1.050789);
      yLeft[sootOff+5] = log(3.5481e-01 / 1.050789);
      yLeft[sootOff+6] = log(2.2160e-13 / 1.050789);
    #endif
    #ifdef PHI_0_95
      yLeft[sootOff  ] = log(1.7151e-12 / 1.042384);
      yLeft[sootOff+1] = log(5.6957e-05 / 1.042384);
      yLeft[sootOff+2] = log(6.1319e-07 / 1.042384);
      yLeft[sootOff+3] = log(3.4763e+03 / 1.042384);
      yLeft[sootOff+4] = log(3.5867e+01 / 1.042384);
      yLeft[sootOff+5] = log(3.7209e-01 / 1.042384);
      yLeft[sootOff+6] = log(2.3240e-13 / 1.042384);
    #endif
    #ifdef PHI_1_05
      yLeft[sootOff  ] = log(1.7738e-12 / 1.035620);
      yLeft[sootOff+1] = log(5.8907e-05 / 1.035620);
      yLeft[sootOff+2] = log(6.3419e-07 / 1.035620);
      yLeft[sootOff+3] = log(3.5954e+03 / 1.035620);
      yLeft[sootOff+4] = log(3.7095e+01 / 1.035620);
      yLeft[sootOff+5] = log(3.8484e-01 / 1.035620);
      yLeft[sootOff+6] = log(2.4035e-13 / 1.035620);
    #endif
    #ifdef PHI_1_10
      yLeft[sootOff  ] = log(1.8717e-12 / 1.028385);
      yLeft[sootOff+1] = log(6.2158e-05 / 1.028385);
      yLeft[sootOff+2] = log(6.6919e-07 / 1.028385);
      yLeft[sootOff+3] = log(3.7938e+03 / 1.028385);
      yLeft[sootOff+4] = log(3.9142e+01 / 1.028385);
      yLeft[sootOff+5] = log(4.0608e-01 / 1.028385);
      yLeft[sootOff+6] = log(2.5362e-13 / 1.028385);
    #endif
    #ifdef PHI_1_15
      yLeft[sootOff  ] = log(2.1376e-12 / 1.011168);
      yLeft[sootOff+1] = log(7.0986e-05 / 1.011168);
      yLeft[sootOff+2] = log(7.6423e-07 / 1.011168);
      yLeft[sootOff+3] = log(4.3326e+03 / 1.011168);
      yLeft[sootOff+4] = log(4.4702e+01 / 1.011168);
      yLeft[sootOff+5] = log(4.6375e-01 / 1.011168);
      yLeft[sootOff+6] = log(2.8964e-13 / 1.011168);
    #endif
  #else
  ...
  flame->UpdateSolutionOnePoint(yLeft, kPrev);
  flame->SetFlameNode(kPrev);
  flame->ComputeProperties(flame->fFlameNode, flame->fFlameNode->temp[kCurr], flame->fFlameNode->Y[kCurr], pressure);
}
#endif
#ifdef V
void UnstrPremPhysUpdateLeftBoundary( void  *object )
...
/*		exit(2);*/
	...
	if (flame->GetSoot())
	{
...
#endif
		{
			...
			{
				yLeft[k+sootOff] = pow(2.0*flame->GetSoot()->nucl_nbrC2, i) * yLeft[sootOff];
			}
		}
#ifdef FRENKLACH
		yLeft[sootOff+nSootMoments-1] = 1.0e-20;
		...
		for ( int i = 0; i < nSootMoments; ++i ) {
			switch ( i ) {
				case 0:
					...
					break;
				default:
					yLeft[i+sootOff] = 18 * yLeft[i+sootOff-1];
					break;
			}
		}
//		flame->GetSoot()->PostIter( flame );
...
//	fprintf( stderr, "M = %g\n", currGrid->GetYLeft()->vec[flame->fMassFlowRate] );
	for ( int i = 0; i < speciesIn; ++i ) {
		f[i] = ( yLeft[i] - MAX( bcLeft[fFirstSpecies+i], 1.0e-15 ) )
			+ rhoY_iV_iPlus[i]
/*			- flame->GetProperties()->GetDensity()->vec[0] * flame->GetSpecies()->GetDiffusivity()->mat[0][i]*/
/*			  * ( currGrid->GetY()->mat[0][fFirstSpecies+i] - yLeft[i] ) / hFirst*/
			/ ( currGrid->GetYLeft()->vec[flame->fMassFlowRate] );
/*			Double	coeff = flame->GetProperties()->GetDensity()->vec[0] * flame->GetSpecies()->GetDiffusivity()->mat[0][i] */
/*					/ ( currGrid->GetYLeft()->vec[flame->fMassFlowRate] * hFirst );*/
/*			f[i] = yLeft[i] - ( bcLeft[fFirstSpecies+i] + coeff * flame->fFlameNode->Y[kNext][i] ) */
/*				/ ( 1.0 + coeff );*/
	}
/*fprintf( stderr, "bcLeft[fFirstSpecies+i] = %g\n", bcLeft[fFirstSpecies+23] );*/
/*fprintf( stderr, "rho = %g\n", flame->GetProperties()->GetDensity()->vec[0] );*/
/*fprintf( stderr, "D = %g\n", flame->GetSpecies()->GetDiffusivity()->mat[0][23] );*/
/*fprintf( stderr, "Y0 = %g\n", currGrid->GetY()->mat[0][fFirstSpecies+23] );*/
/*fprintf( stderr, "hFirst = %g\n", hFirst );*/
/*fprintf( stderr, "M = %g\n", currGrid->GetYLeft()->vec[flame->fMassFlowRate] );*/
/*fprintf( stderr, "YL = %g\n", yLeft[23] );*/
//	fprintf( stderr, "rhoY_iV_iPlus[0] = %g\n", rhoY_iV_iPlus[0] );
//	fprintf( stderr, "Y[0] = %g\n", yLeft[23] );
//	fprintf( stderr, "bc[0] = %g\n", bcLeft[fFirstSpecies+23] );
...
//	constDiffMinus = - 1.0 / (nodeInfo->hm * hPlushm);
//	constDiffPlus = - 1.0 / (nodeInfo->h * hPlushm);
	Double	rhoY_iV_iMin, rhoY_iV_iPlus, YCW;
	for ( i = 0; i < nSpeciesInSystem; ++i ) {
//		diffMinus = constDiffMinus * ( mixDensity[kPrev] * diffusivityPrev[i] + mixDensity[kCurr] * diffusivity[i] );
//		diffPlus = constDiffPlus * ( mixDensity[kCurr] * diffusivity[i] + mixDensity[kNext] * diffusivityNext[i] );
...
void TUnstrPremFlamePhys::PrintProdRatePAH(FILE *fp )
{
  int k, s;
  int nPAH = 8;
