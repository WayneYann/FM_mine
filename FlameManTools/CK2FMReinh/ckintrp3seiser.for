...
C
      ...
      PARAMETER (MDIM=10,KDIM=1200,MKDIM=MDIM*KDIM,IDIM=6000,LSYM=16,

     1           NPAR=3, NPIDIM=IDIM*NPAR, NPC=5, NPCP2=NPC+2, MAXTP=3,

     2           NTR=MAXTP-1, NKTDIM=NTR*NPCP2*KDIM, MAXSP=40, MAXTB=10,
...
C
     ...
     1          KEY(5)*4, LINE*180, IUNITS*80, AUNITS*4, EUNITS*4,

     2          UPCASE*4, VERS*(LSYM), PREC*(LSYM),

     3          tfname*64, ifname*64, ofname*64, lfname*64
...
c               open files - revised 3/22/99 by BDH

c

      iocheck=0 

    9 if(iocheck.ne.0) write(*,10) iocheck

   10 format(1x,'error: iocheck=',i4)

      if(iocheck.eq.6416) write(*,*) 'file does not exist'

      write(*,*)'thermo data file name = ? (q to quit)'

      read(*,'(a64)') tfname

      if (tfname.eq.'q'.or.tfname.eq.'Q') stop

      open(lthrm,file=tfname,status='old',form='formatted',err=9,

     &     iostat=iocheck)

   11 if(iocheck.ne.0) write(*,10) iocheck

      if(iocheck.eq.6416) write(*,*) 'file does not exist'

      write(*,*)'input model file name = ? (q to quit)'

      read(*,'(a64)') ifname

      if (ifname.eq.'q'.or.ifname.eq.'Q') stop

      open(lin,file=ifname,status='old',err=11,iostat=iocheck)

   13 if(iocheck.ne.0) write(*,10) iocheck

      if(iocheck.eq.6415) then 
...
C        if(answer.eq.'y'.or.answer.eq.'Y') then
          ...
          open(lout,file=ofname,err=13,form='formatted',iostat=iocheck)

          go to 14

        else

          write(*,*)'error reading, try again'

        endif

      endif

      write(*,*)'interpreter ascii output file name = ?'

      read(*,'(a64)') ofname

      open(lout,file=ofname,status='new',err=13,iostat=iocheck)

   14 if(iocheck.ne.0) write(*,10) iocheck
...
C        if(answer.eq.'y'.or.answer.eq.'Y') then^M 
        if(1.eq.1) then
          open(linc,file=lfname,err=14,form='unformatted',
     ...
     &                                             iostat=iocheck)
      ...
      write(*,*)'CHEMKIN link file name = ?'

      read(*,'(a64)') lfname

      open(linc,file=lfname,status='new',form='unformatted',err=14,
...
C      OPEN (LOUT, FORM='FORMATTED', STATUS='UNKNOWN', FILE='chem.out')
...
C      OPEN (LIN, FORM='FORMATTED', STATUS='UNKNOWN', FILE='chem.inp')
...
C            OPEN (LTHRM, FORM='FORMATTED', STATUS='UNKNOWN', 
...
C      OPEN (LINC, FORM='UNFORMATTED', STATUS='UNKNOWN', 
...
C
 ...
 1095 FORMAT (6X,'Error...no duplicate declared for reaction no.',I4)
...
C
     ...
     2          IRNU(*), RNU(MAXSP,*), IPLUS(20)
      ...
      CHARACTER*180 ISTR, IREAC, IPROD, ISPEC, INAME, ITEMP
...
C 1900 FORMAT (I4,'. ', A, T53, 1PE8.2, 2X, 0PF5.1, 2X, F9.1)

 1900 FORMAT (I4,'. ', A, T53, 1PE10.3, 2X, 0PF7.3, 2X, F11.3)
 1920 FORMAT (6X,A)
...
C 1900 FORMAT (6X, A, T53, 1PE8.2, 2X, 0PF5.1, 2X, F9.1)

 1900 FORMAT (6X, A, T53, 1PE10.3, 2X, 0PF7.3, 2X, F11.3)

 3040 FORMAT (9X,'Reverse Landau-Teller parameters: B=',E12.5,

     1           ', C=',E12.5)

 3050 FORMAT (6X,'Low pressure limit:',3E13.4E3)

 3060 FORMAT (6X,'SRI centering:     ',3E13.4E3)

 3070 FORMAT (6X,'SRI centering:     ',5E13.4E3)

 3080 FORMAT (6X,'TROE centering:    ',3E13.4E3)

 3090 FORMAT (6X,'TROE centering:    ',4E13.4E3)
